 ...
 *
 * Зачем: контент, который появляется или заменяется внутри анимируемого
 * (transform от motion) контейнера в результате асинхронного колбэка
 * (Firebase auth, Firestore), на iOS Safari иногда остаётся
 * «непрокрашенным»: DOM и layout корректны, тапы обрабатываются,
